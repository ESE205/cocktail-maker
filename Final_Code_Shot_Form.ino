...
int lightPen1 = A1 ; //define a pin for Photo resistor of bottle 1
int lightPen2 = A2 ; //define a pin for Photo resistor of bottle 2
int lightPen3 = A3 ; //define a pin for Photo resistor of bottle 3
...
void loop() {
  ...
  switch (drink) {//Selction of drink. Will lead into making a drink
    ...
    case checkbottles://If bottles are empty
    ...
    case drinkmaking:
      ...
      if (d == "d1" && strength == "regular") {
        while (scale.get_units() < 15) {
          digitalWrite (Vodka, LOW);                                        //pump 1 on
        }
        digitalWrite (Vodka, HIGH);                                         // Turns pump 1 off
        while (scale.get_units() < 100 && scale.get_units() >= 14) {
          digitalWrite (Gingerale, LOW);                                      //pump 2 on
        }
        digitalWrite(Gingerale, HIGH);                                      // Pump 2 off
        lcd.print("Drink Made");//Prints drink is ready
      ...
      if (d == "d3" && strength == "strong") {
        ...
        digitalWrite(Vodka, HIGH);                                          //pump 1 off
        while (scale.get_units() < 65 && scale.get_units() >=29) {
          digitalWrite (CranMango, LOW);                                    //pump 3 on
        }
        digitalWrite (CranMango, HIGH);                                      //pump 3 off
        while (scale.get_units() < 100 && scale.get_units() >= 64) {
      ...
      if (d == "d3" && strength == "regular") {
        ...
        digitalWrite(Vodka, HIGH);                                          //pump 1 off
        while (scale.get_units() < 57 && scale.get_units() >= 14) {
          digitalWrite (CranMango, LOW);                                    //pump 3 on
        }
        digitalWrite (CranMango, HIGH);                                      //pump 3 off
        while (scale.get_units() <100  && scale.get_units() >= 56) {
      ...
      if (d == "d3" && strength == "virgin") {
        while (scale.get_units() < 50) {
          digitalWrite (CranMango, LOW);                                    // pump 3 on
        }
        digitalWrite(CranMango, HIGH);                                      //pump 3 off
        while (scale.get_units() < 100 && scale.get_units() >50) {
          digitalWrite (Gingerale, LOW);                                    //pump 2 on
        }
